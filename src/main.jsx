...
function CartDrawer({ cart, changeQuantity, removeFromCart, total, isCombo, onClose, onCatalog }) {
  return <div className="drawer-layer" onClick={onClose}><aside className="cart-drawer" onClick={(event) => event.stopPropagation()}>
    ...
    {cart.length === 0 ? <div className="empty-cart"><div className="empty-icon"><ShoppingBag /></div><h3>Tu carrito está vacío</h3><p>Descubre productos para crear tu ritual de bienestar.</p><button className="button button-primary" onClick={onCatalog}>Explorar catálogo <ArrowRight size={16} /></button></div> : <>
      ...
      <Link to="/pedido" className="button button-primary button-wide" onClick={onClose}>Completar pedido <ArrowRight size={16} /></Link>
...
function CheckoutPage() {
  ...
  return <Layout><section className="checkout-page container"><div className="checkout-intro"><Link to="/catalogo" className="back-link">← Seguir comprando</Link><span className="eyebrow">Pedido Botané</span><h1>Completa tu <em>pedido.</em></h1><p>Revisa tus productos y déjanos los datos para coordinar la entrega.</p></div><div className="order-summary"><h2>Productos agregados</h2>{order.items.map((item) => <div className="order-summary-item" key={item.id}><img src={item.image} alt="" /><span><strong>{item.name}</strong><small>{item.quantity} x {formatCOP(item.price)}</small></span><b>{formatCOP(item.price * item.quantity)}</b></div>)}{order.isCombo && <p className="combo-note"><Check size={16} /> Pack completo aplicado: ahorras $12.000</p>}<div className="order-summary-total"><span>Total</span><strong>{formatCOP(order.total)}</strong></div></div><Checkout product={order.items[0]} quantity={order.items[0].quantity} cart={order.items} total={order.total} isCombo={order.isCombo} page onClose={() => navigate('/catalogo')} /></section></Layout>
}

function Checkout({ product, quantity, cart, total, isCombo, page = false, onClose }) {
  const navigate = useNavigate()
  ...
  const submit = async (event) => {
    ...
    if (phoneDigits.length !== 10 || !phoneDigits.startsWith('3')) { setSubmitting(false); setError('Ingresa un celular colombiano válido de 10 dígitos.'); return }
    if (form.document.trim().length < 5 || form.addressInfo.trim().length < 8) { setSubmitting(false); setError('Completa un documento y una dirección válida.'); return }
    const order = getOrderDetails({ cart, product, quantity })
    const payload = buildFormspreePayload({ form, order })
    try { const response = await fetch(FORM_ENDPOINT, { method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); if (!response.ok) throw new Error('formspree'); clearCart(); setSuccess(true) } catch { setError('No pudimos enviar el pedido. Revisa tu conexión o escríbenos por WhatsApp.') } finally { setSubmitting(false) }
  }
